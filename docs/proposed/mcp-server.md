...
5. run_tests(path)              → execute the tests, get pass/fail results
6. IF failures:
     dump_results()            → get detailed failure data (logs, assertions, traffic)
     AI reads failures, fixes definitions
     GOTO 4
7. Done — tests pass
```
...
Runs `dokkimi dump`, writes the output to a local file, and returns the file path. The AI then reads the file using its native file tools (which already handle large files with offset/limit).
...
**When the AI calls this:** After `run_tests` completes. Use `failedOnly: true` to focus on debugging failures. Use `failedOnly: false` (or omit) to analyze passing tests too — useful for verifying traffic patterns, reviewing what assertions actually matched, or understanding the full picture before writing more tests.
