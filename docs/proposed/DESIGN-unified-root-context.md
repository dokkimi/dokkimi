...
Users must know which action type produced the result *and* whether they're in extract or assertion context to write the correct path. The docs explicitly warn: "don't use `response.body` in extract paths or `$.body` in assertion paths."
...
- "The service logged an error *after* the DB query failed" — temporal ordering across log types
...
**Internal data stays as-is.** The log buffers, variable context, and sidecar log ingestion don't change. `AssembleRootContext` is a *view* — it builds one shallow map of references to data that already exists in memory.
